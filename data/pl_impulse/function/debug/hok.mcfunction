...
execute store result score #pliH. _ if entity @e[tag=hok,scores={_=1}]
execute if score #pliH. _ matches 2.. as @e[tag=hok,scores={_=1},limit=1] on passengers run kill @s
execute if score #pliH. _ matches 2.. run kill @e[tag=hok,scores={_=1},limit=1]
...
data modify storage pm_motion: in set value {size:6,max_reflect:0,velocity:100,max_distance:50,max_time:100,c_drag:0,entity:false,events:{trail:"particle dust{color:[1,1,1],scale:1} ~ ~ ~ 0 0 0 0.001 1",last:"function pl_impulse:debug/hit"}}
